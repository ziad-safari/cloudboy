...
CPU::CPU(emu* myemu) : regs(), fetched_data(0), mem_dest(0), dest_is_mem(false), opcode(0), EMU{myemu} {
    regs.pc = 0x100;
    regs.a = 0x01;
}
void CPU::fetch_instruction() {

    opcode = bus_read(*EMU, regs.pc++);
...
bool CPU::step() {
    if (!halted) {
        ...
        printf("%04X: %-7s (%02X %02X %02X) A: %02X B: %02X C: %02X\n", 
            ...
            bus_read(*EMU, pc + 1), bus_read(*EMU, pc + 2), regs.a, regs.b, regs.c);
