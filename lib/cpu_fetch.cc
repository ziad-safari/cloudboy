...
void CPU::fetch_data() {
    ...
    switch(cur_inst->mode) {
        ...
        case AM_D16: {
            u16 lo = bus_read(regs.pc);
            emu_cycles(1);

            u16 hi = bus_read(regs.pc + 1);
            emu_cycles(1);
        ...
        case AM_R_MR: {
            ...
            fetched_data = bus_read(addr);
            emu_cycles(1);

        } return;
        // Load value into reg from the byte at HL and then increment HL
        case AM_R_HLI:
            fetched_data = bus_read(cpu_read_reg(cur_inst->reg_2));
            emu_cycles(1);
        ...
        case AM_A8_R:
            mem_dest = bus_read(regs.pc) | 0xFF00;
            dest_is_mem = true;        
            emu_cycles(1);
            regs.pc++;
            return;
        ...
        case AM_MR:
            mem_dest = cpu_read_reg(cur_inst->reg_1);
            dest_is_mem = true;
            fetched_data = bus_read(cpu_read_reg(cur_inst->reg_1));
            emu_cycles(1);
            return;
